#!/usr/bin/env node
/**
 * Validates all GSD skills for Vibe compatibility
 * Checks STRUCTURE without executing code
 */
...
// List of valid Vibe tools (from Vibe documentation)
...
function validateSkill(skillPath) {
  ...
  // 3. Validate name - MUST have gsd- prefix
  if (!frontmatter.name) {
  ...
  // 6. Check process block
  if (!content.includes('>>') || !content.includes('<<')) {
    warnings.push(`Missing process block (>> ... <<)`);
  }

  // 7. Check process block structure
  ...
  // 14. Check slash commands (should not be present anymore)
...
function main() {
  ...
  for (const dir of skillDirs) {
    ...
    // Only show warnings with --verbose (or in summary)
