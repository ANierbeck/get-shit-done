...
type: reference
category: gsd
description: GSD Verification patterns reference documentation
version: 1.0
focused: true
on-demand: true
---
...
return No messages  // Always shows "no messages"

// Wrong state rendered:
const [messages, setMessages] = useState([])
return {otherData.map(...)}  // Uses different data
```
...
check_substantive() {
  ...
  local min_lines="$2"
  local pattern="$3"
  local lines=$(wc -l /dev/null || echo 0)
